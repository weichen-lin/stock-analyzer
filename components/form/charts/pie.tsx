...
export default function BasicPie() {
  ...
  const getValue = (e: IStockConfig, total: string) => {
    if (currency(e.shares).value === 0 || currency(e.averageCost).value === 0) return 0
    return currency(e.shares, { precision: 4 }).multiply(e.averageCost).divide(currency(total)).multiply(100).value
  }
  ...
  const remain = { label: '未投入', value: 100 - stocks.reduce((a, b) => a + b.value, 0), id: stocks.length }
